...
export default class DuTable extends React.Component {
    ...
    tableFormatHearder = data => {
                    ...
                    render: (text, record) => {
                        return (
                            <div className="page-table-handle">
                                {
                                    Array.isArray(text) && text.map((item, key) => {
                                        let isArray = Array.isArray(item.value);
                                        return <a key = {key}
                                            disabled = {isArray ? !item.value[1] : false}
                                            onClick={() => this.actionOperate(item.key, item.handleType, record, 'click')}
                                        >{item.value}</a>;
                                    })
                                }
                            </div>
        ...
        this.setState({
            columns: this.props.columns ? this.props.columns : titleArray
        });
    }
